...
//
// All given configuration files have to be loaded successfully, otherwise an error is returned together with an empty configuration.
func Load(configFiles []string) (Config, error) {
	...
	for _, configFile := range configFiles {
		...
		if err != nil {
			return Config{}, fmt.Errorf("error reading configuration file %q: %w", configFile, err)
		}
		...
		if err != nil {
			return Config{}, fmt.Errorf("error unmarshaling json from configuration file %q: %w", configFile, err)
		}
	}
